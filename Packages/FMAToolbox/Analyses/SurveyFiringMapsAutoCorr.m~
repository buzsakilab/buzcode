function [maps,stats] = SurveyFiringMapsAutoCorr(units,varargin)

%SurveyFiringMapsAutoCorr - Compute and plot autocorrelogram maps for all conditions.
%
%  USAGE
%
%    [maps,stats] = SurveyFiringMapsAutoCorr(units,<options>)
%
...
if mod(length(varargin),2) ~= 0,
	error('Incorrect number of parameters (type ''help <a href="matlab:help SurveyFiringMapsAutoCorr">SurveyFiringMapsAutoCorr</a>'' for details).');
end

% Parse options
for i = 1:2:length(varargin),
	if ~ischar(varargin{i}),
		error(['Parameter ' num2str(i+1) ' is not a property (type ''help <a href="matlab:help SurveyFiringMapsAutoCorr">SurveyFiringMapsAutoCorr</a>'' for details).']);
	end
	switch(lower(varargin{i})),
		case 'minv',
			minv = varargin{i+1};
			if ~isdscalar(minv,'>=0'),
				error('Incorrect value for property ''minv'' (type ''help <a href="matlab:help SurveyFiringMapsAutoCorr">SurveyFiringMapsAutoCorr</a>'' for details).');
			end
		case 'pixel',
			pixel = varargin{i+1};
			if ~isdscalar(pixel,'>0'),
				error('Incorrect value for property ''pixel'' (type ''help <a href="matlab:help SurveyFiringMapsAutoCorr">SurveyFiringMapsAutoCorr</a>'' for details).');
			end
		case 'show',
			show = varargin{i+1};
			if ~isstring_FMAT(show,'on','off'),
				error('Incorrect value for property ''show'' (type ''help <a href="matlab:help SurveyFiringMapsAutoCorr">SurveyFiringMapsAutoCorr</a>'' for details).');
			end
		otherwise,
			error(['Unknown property ''' num2str(varargin{i}) ''' (type ''help <a href="matlab:help SurveyFiringMapsAutoCorr">SurveyFiringMapsAutoCorr</a>'' for details).']);
	end
end
...
if ~isempty(minv),
	if isempty(pixel),
		error(['Missing pixel size for minimum velocity (type ''help <a href="matlab:help SurveyFiringMapsAutoCorr">SurveyFiringMapsAutoCorr</a>'' for details).']);
	end
		...
		for i = 1:nSessions,
			...
			if strcmp(show,'on'),
				...
				%PlotColorMap(map.rate,map.time,'ydir','reverse','bar','off','cutoffs',[0 10],'gamma',2);
					autocorrelogram = xcorr2(map.rate-mean(map.rate(:)));
					autocorrelogram = autocorrelogram ./ max(max(autocorrelogram));
					PlotColorMap(autocorrelogram,1,'ydir','reverse','bar','off','cutoffs',[-1 1],'gamma',2);
