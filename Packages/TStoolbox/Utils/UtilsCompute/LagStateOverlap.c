/* LC = LagStateOverlap(Q, maxbin, intervals)
...
static double overlap(int i, int j, double *mean, double *std)
{
  int k, nRow = nBins;
  double cov = 0.;
  double k1 = 0., k2 = 0.;
  ...
  for(k = 0; k < nCells; k++)
    {
      cov += (MAT_ITEM(Q, i, k) > 0) * (MAT_ITEM(Q, j, k) > 0);
      k1 += (MAT_ITEM(Q, i, k) > 0);
      k2 += (MAT_ITEM(Q, j, k) > 0);
    }
