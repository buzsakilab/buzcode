function EV_idx = ReactRmatrices_idx(filebase, cell_list, epochs_start, epochs_end, restrict_times)
% ReactRmatrices 
%
...
%  keyboard
  ...
  % compute the per cell reactivation indices 
  
  for i = 1:length(S)
    
    idx = find((~findOnSameTrodeMatrix(cellnames)) & (X > Y) & ...
	       (X ~= i) & (Y ~= i));
    rp_s1 = full(cQ_s1(idx));
    rp_s2 = full(cQ_s2(idx));
    rp_m = full(cQ_m(idx));
    
    [ev evr] = ReactEV(rp_s1, rp_s2, rp_m);
    EV_idx(i) = EV-EVr-(ev-evr);
