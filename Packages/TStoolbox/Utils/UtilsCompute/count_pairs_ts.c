/* count_pairs.c: MEX file, "engine" for the spike_pairs
   ...
   pairs_x
   pairs_y 
   pairs_phi: 3 n_combs X 2 cell array of arrays, containing the
   locations 
                 ...
                 *prhs[])
{
  ...
  double ***pairs_ts_ptr, ***pairs_x_ptr, ***pairs_y_ptr,
    ***pairs_phi_ptr;
  int **pairs_count;
  int **pairs_last_ts;
  ...
  enum {PAIRS_FREQ, PAIRS_TABLE, PAIRS_TS, PAIRS_X,
	PAIRS_Y, PAIRS_PHI
  } out_arg_order;
  ...
  plhs[PAIRS_X] = mxCreateCellMatrix(n_combs, 2);

  plhs[PAIRS_Y] = mxCreateCellMatrix(n_combs, 2);

  plhs[PAIRS_PHI] = mxCreateCellMatrix(n_combs, 2);
  ...
  pairs_x_ptr = (double ***)mxCalloc(2, sizeof(double **));
  pairs_y_ptr = (double ***)mxCalloc(2, sizeof(double **));
  pairs_phi_ptr = (double ***)mxCalloc(2, sizeof(double **));
  pairs_count = (int **)mxCalloc(2, sizeof(int *));
  
  for(i = 0; i < 2; i++)
    {
      pairs_ts_ptr[i] = (double **)mxCalloc(n_combs, sizeof(double **));
      pairs_x_ptr[i] = (double **)mxCalloc(n_combs, sizeof(double *));
      pairs_y_ptr[i] = (double **)mxCalloc(n_combs, sizeof(double *));
      pairs_phi_ptr[i] = (double **)mxCalloc(n_combs, sizeof(double *));
  ...
  for(i = 0; i < n_combs; i++)
	...
	tmp = mxCreateNumericMatrix(n_pairs, 1, mxDOUBLE_CLASS,
				    mxREAL);
	pairs_x_ptr[j][i] = mxGetPr(tmp);
	
	mxSetCell(plhs[PAIRS_X], 
		  mxCalcSingleSubscript(plhs[PAIRS_X], 2, midx),
		  tmp);


	tmp = mxCreateNumericMatrix(n_pairs, 1, mxDOUBLE_CLASS,
				    mxREAL);
	pairs_y_ptr[j][i] = mxGetPr(tmp);
	
	mxSetCell(plhs[PAIRS_Y], 
		  mxCalcSingleSubscript(plhs[PAIRS_Y], 2, midx),
		  tmp);
	...
	tmp = mxCreateNumericMatrix(n_pairs, 1, mxDOUBLE_CLASS,
				    mxREAL);
	pairs_phi_ptr[j][i] = mxGetPr(tmp);
	
	mxSetCell(plhs[PAIRS_PHI], 
		  mxCalcSingleSubscript(plhs[PAIRS_PHI], 2, midx),
		  tmp);
      }
	    ...
	    jx = idxs[j];
		{
		  ...
		  if(cx >= 0 && (!omit_double ||
			...
			tstamp[i];
		      ...
		      pairs_x_ptr[px][cx][pairs_count[px][cx]] = 
			x_pos[i];

		      pairs_y_ptr[px][cx][pairs_count[px][cx]] = 
			y_pos[i];

		      pairs_phi_ptr[px][cx][pairs_count[px][cx]] = 
			phi_pos[i];
  ...
  for(i = 0; i < 2; i++)
    {
      ...
      mxFree((void *)pairs_x_ptr[i]);
      mxFree((void *)pairs_y_ptr[i]);
      mxFree((void *)pairs_phi_ptr[i]);
  ...
  mxFree((void *)pairs_x_ptr);
  mxFree((void *)pairs_y_ptr);
  mxFree((void *)pairs_phi_ptr);
