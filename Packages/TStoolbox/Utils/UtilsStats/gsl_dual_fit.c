...
/* [A1, lambda1, b, A2, lambda2] = exp_fit_gsl(y, sigma)
   ...
   y(i) = A1 * exp(- lambda1 * t) + A1 * exp(- lambda1 * t) + b on data y with sdev sigma
*/
  ...
  for (i = 0; i < n; i++)
    {
      ...
      gsl_matrix_set (J, i, 0, e2/s); 
      gsl_matrix_set (J, i, 1, -t * A2 * e2/s);
                 ...
                 *prhs[])
{
  ...
  double x_init[5] = { 1.0, 0.05, 0.0, 1.0, 1.0};
