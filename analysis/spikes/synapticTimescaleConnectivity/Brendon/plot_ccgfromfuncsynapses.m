function plot_ccgfromfuncsynapses(S,cellidx1,cellidx2)
...
BinMs = 1;
duration = 300;%timepoints
SampleRate = 10000;
numSampsBinSize = BinMs/1000*SampleRate;

times1 = TimePoints(S{cellidx1});
times2 = TimePoints(S{cellidx2});
res = [times1' times2'];
clu = [ones(size(times1))' 2*ones(size(times2))'];
HalfBins = round(duration/numSampsBinSize);


[ccgR, tR] = CCG_In(res,clu,numSampsBinSize,HalfBins,SampleRate,[1 2],'hz');
