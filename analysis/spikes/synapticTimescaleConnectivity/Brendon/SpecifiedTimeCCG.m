function [ccg,bintimes] = ShortTimeCCG(times1,times2,startstops,varargin)

% SpecifiedTimeCCG - Time-varying auto/cross-correlograms of point 
% processes over specified times.
% >!Based almost exactly on ShortTimeCCG by Zugaro in FMAToolbox... not
% recommented yet.  Brendon Watson 2014.   
%
...
%    startstops     paired start and stop times for each epoch of interest:
%                   column1 is start times, column2 is matched stop times
...
%     'duration'    duration/total width (in s) of each xcorrelogram (default = 2)
...
%                   is a time bin
%    bintimes       center times of bins for xcorrelograms
%
%  SEE
%
...
% 	x(i,1) = t;
% ok = InIntervals(times,t+[-0.5 0.5]*window);%Find which t's fall in this interval
	
for i = 1:size(startstops,1);
    thisstart = startstops(i,1);
    thisstop = startstops(i,2);
    
    ok = InIntervals(times,[thisstart thisstop]);%Find which t's fall in this interval
    
    if sum(ok) < minEvents,
		ccg(1:(2*halfBins+1),i) = nan;%assign null data
	else
...
bintimes = (-duration/2:binSize:duration/2)';
