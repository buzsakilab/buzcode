function z = xmltools_old( arg, out_file, varargin)
% xmltools_old - tools for managing xml data sets
...
% use :
%  z = xmltools_old('filename.xml'); read an xml file and store it into z
%  xmltools_old(z,'filename.xml'); write z into the file
%  xmltools_old(z,'get','tag-name'); returns only subset of z child which name is tag-name
%  xmltools_old(z,'get-attrib', 'attrib-name')
%  xmltools_old(z,'get','tag-name', 'attribs'|'value'|'child');
%
...
if ~isstr(arg)
  ...
  if length(varargin) >= 1 %& ~exists(arg,'file')
    ...
    switch upper( cmode)
     ...
     case 'GET'
      ...
      if ~isfield(z, next)
        error('xmltools_old:GET', 'For child selection, structured first argument is needed');
      end
      tag_name = (varargin{1});
      ...
      if length(varargin) == 2
        switch upper( varargin{2})
         ...
         case 'CHILD'
          z = [z.child];
         otherwise
          error('xmltools_old:GET', 'get mode <%s> is not defined, use one of <attribs>|<value>|<child>', upper(varargin{2}));
        end
      end
      %>
     ...
     case 'GET-ATTRIB'
      ...
      error('xmltools_old:GET-ATTRIB', 'no attribute found'); %, varargin{1});
      %>
...
while ~eot & ~isempty(udeblank(deblank(str)))
  ...
  if isempty(f_end) & isempty(f_beg)
    ...
    if ~strcmp(lower(current_tag), '?xml') & ~isempty(current_tag)
      error('xmltools_old:parse_xml', 'malformed xml string (current [%s])', current_tag);
    else
  ...
  if f_end <= f_beg
    ...
    if isempty(f_end)
      error('xmltools_old:parse_xml', 'malformed xml string : never ending tag [%s] encountered', current_tag);
    end
    f_end = f_end(1);
    str     = new_tag(f_end+1:end); % reste
    new_tag = new_tag(1:f_end-1);
    if ~strcmp((new_tag), (current_tag))
      error('xmltools_old:parse_xml', 'malformed xml string : [%s] not properly closed (closing [%s] encountered)', current_tag, new_tag);
    end
    if Verbose
    ...
    f_end = strfind(new_tag,'>');
    if isempty(f_end)
      error('xmltools_old:parse_xml', 'malformed xml string : never ending tag encountered');
    end
    f_end   = f_end(1);
...
if isfield(xml_struct, 'tag')
  ...
  if ~isfield(xml_struct, 'attribs')
    error('xmltools_old:write_xml', 'malformed MATLAB xml structure : tag without attribs');
  end
  ...
  if ~isfield(xml_struct, next)
    error('xmltools_old:write_xml', 'malformed MATLAB xml structure : tag without %s', next);
  end
  if ~isfield(xml_struct, 'value')
    error('xmltools_old:write_xml', 'malformed MATLAB xml structure : tag without value');
  end
  if xml_struct.tag(1) == '?'
    fprintf(fid, '?>\n');
    closed_tag = 1;
...
if ~isstruct( zo)
  ...
  error('xmltools_old:GET-TEG', 'problem in finding tag <%s> under one <%s>', tag_name, tn);
end
z = [ zo.anext ];
%>*
