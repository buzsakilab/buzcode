function Intan_rec_info = read_Intan_RHD2000_file_Peter(path_in)
...
if ~exist(filename)
    error(['PP: Intan info file does not exist (' filename ')'])
end
tic;
...
Intan_rec_info.notes = notes;
% move_to_base_workspace(frequency_parameters);
Intan_rec_info.frequency_parameters = frequency_parameters;

if (num_amplifier_channels > 0)
%     move_to_base_workspace(amplifier_channels);
    Intan_rec_info.amplifier_channels = amplifier_channels;
    if (data_present)
%         move_to_base_workspace(amplifier_data);
        Intan_rec_info.amplifier_data = amplifier_data;
%         move_to_base_workspace(t_amplifier);
        Intan_rec_info.t_amplifier = t_amplifier;
    end
%     move_to_base_workspace(spike_triggers);
    Intan_rec_info.notes = spike_triggers;
end
if (num_aux_input_channels > 0)
%     move_to_base_workspace(aux_input_channels);
    Intan_rec_info.aux_input_channels = aux_input_channels;
    if (data_present)
%         move_to_base_workspace(aux_input_data);
        Intan_rec_info.aux_input_data = aux_input_data;
%         move_to_base_workspace(t_aux_input);
        Intan_rec_info.t_aux_input = t_aux_input;
    end
end
if (num_supply_voltage_channels > 0)
%     move_to_base_workspace(supply_voltage_channels);
    Intan_rec_info.supply_voltage_channels = supply_voltage_channels;
    if (data_present)
%         move_to_base_workspace(supply_voltage_data);
        Intan_rec_info.supply_voltage_data = supply_voltage_data;
%         move_to_base_workspace(t_supply_voltage);
        Intan_rec_info.t_supply_voltage = t_supply_voltage;
    end
end
if (num_board_adc_channels > 0)
%     move_to_base_workspace(board_adc_channels);
    Intan_rec_info.board_adc_channels = board_adc_channels;
    if (data_present)
%         move_to_base_workspace(board_adc_data);
        Intan_rec_info.board_adc_data = board_adc_data;
%         move_to_base_workspace(t_board_adc);
        Intan_rec_info.t_board_adc = t_board_adc;
    end
end
if (num_board_dig_in_channels > 0)
%     move_to_base_workspace(board_dig_in_channels);
    Intan_rec_info.board_dig_in_channels = board_dig_in_channels;
    if (data_present)
%         move_to_base_workspace(board_dig_in_data);
        Intan_rec_info.board_dig_in_data = board_dig_in_data;
%         move_to_base_workspace(t_dig);
        Intan_rec_info.t_dig = t_dig;
    end
end
if (num_board_dig_out_channels > 0)
%     move_to_base_workspace(board_dig_out_channels);
    Intan_rec_info.board_dig_out_channels = board_dig_out_channels;
    if (data_present)
%         move_to_base_workspace(board_dig_out_data);
        Intan_rec_info.board_dig_out_data = board_dig_out_data;
%         move_to_base_workspace(t_dig);
        Intan_rec_info.t_dig = t_dig;
    end
end
...
%         move_to_base_workspace(temp_sensor_data);
        Intan_rec_info.temp_sensor_data = temp_sensor_data;
%         move_to_base_workspace(t_temp_sensor);
        Intan_rec_info.t_temp_sensor = t_temp_sensor;
    end
end
