...
if abs(slope) < 4 * size(Pr,1) ./ size(Pr,2)% rise/run limit to calc integral (must be in the frame)
    for i=1:length(xx)
...
%             curve_fit(i) = nan;
    ...
    integral = nanmean(curve);
%     integral = nanmean(curve_fit);
else
    integral = NaN;
    slope = NaN;
end
