classdef max_correlation_coefficient_CL_circular
...
%==========================================================================    
  ...
  properties 
    ...
    methods 
        ...
        % constructor 
        function cl = max_correlation_coefficient_CL_circular
        end
        ...
        function cl = train(cl, XTr, YTr)  
            ...
            for i = 1:length(unique_labels)
                template(:, i) = circ_mean(XTr(:, (YTr == unique_labels(i))), [], 2);
            end
...
%                  mean_subtracted_templates = cl.templates - repmat(circ_mean(cl.templates), [size(cl.templates, 1) 1]);
                 mean_subtracted_templates = (cl.templates - repmat(circ_mean(cl.templates), [size(cl.templates, 1) 1]));
%                  mean_subtracted_XTe = XTe - repmat(circ_mean(XTe), [size(XTe, 1) 1]);
                 mean_subtracted_XTe = (XTe- repmat(circ_mean(XTe), [size(XTe, 1) 1]));
                ...
                template_corrcoeffs  = -1 .* circ_dist(repmat(cl.templates, [size(XTe, 2), 1]), repmat(XTe', [1 size(cl.templates, 2)])).^2;   
