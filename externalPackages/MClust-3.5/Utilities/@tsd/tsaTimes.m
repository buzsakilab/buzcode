...
if isempty(strmatch('units',fieldnames(tsa)))
    warning('TSD:units','tsa units not specified: assuming tsa units = sec ' )
...
switch (unitflag)
    case 'sec'
        switch unit
            ...
            case 'ms'
                R = R/1000;
            otherwise
                warning('TSD:units','tsa has invalid units: no conversion possible' );
        end
    ...
    otherwise
        warning('TSD:units','Convert called with invalid unitflag: no conversion possible');
end
