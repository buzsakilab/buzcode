...
%   $Revision: 1.6.4.11 $  $Date: 2004/12/24 20:46:49 $
...
if ischar(xtype)
    ...
    case 'data'
        ...
        X0 = (X - repmat(mean(X),n,1)) ./ repmat(std(X),n,1); % standardize to cor matrix
...
%          if any(s <= eps(class(s))^(3/4) * max(s))
    ...
    case 'covariance'
        ...
        diagX = diag(X);
        if (size(X,1) ~= d) | any(any(abs(X - X') > 10^-6)) %10*eps(abs(max(diagX)))))
            error('stats:factoran:BadCovariance',...
