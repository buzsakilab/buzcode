...
double  rcs[max_c][max_s];	/* prob distr mean rates */
double  acs[max_c][max_s];	/* prob distr nonzero rates */
double  ra[max_c][max_s];	/* rate averages */
double  ra_true[max_c][max_s];	/* rate averages before rescaling */
double  sd[max_c][max_s];	/* rate variances */
double  f0[max_c][max_s];	/* fraction of zero rates */
...
int     FREQ = False;
double  timewin = 1000.;
double  sp[max_c], lsp[max_c];	/* sparsity and its log */
double  It[max_c], It_av;	/* info derivative */
int     seed;
int     infors()
{
   ...
   double  r_a1[max_c], r_a2[max_c];	/* averages for sparseness */
   double  r_a1_true[max_c];	/* before rescaling  */
   double  r_a1_true_max[max_c];
   double  r_a1_true_min[max_c];
   double  Qsq[max_s][max_s], q1;	/* quantized probability table */
   double  Qq[max_s];		/* quantized Prob(s_q) */
   double  Psq[max_s][max_s], p1;	/* probability table */
   double  Pq[max_s];		/* Prob(s_q) */
   int     nrb[max_c][max_s][MAX_B];	/* number of rates per bin */
   int     nrb0[max_c][MAX_B];	/* total number of rates per bin */
   ...
   double  I_means[max_s];
   double  dp;			/* probability of one trial */
   int     c, s, t, b;		/* cell,stimulus,trial,bin */
   char    outfile_name[80], inffile_name[80], rfile_name[80];
   char    mrfile_name[80], pltfile_name[80];
   int     nt;
   double  rt;
...
void    Read_Input(void)
{
   ...
   nr = (int *) mxMalloc(num_s * sizeof(int));

   r = (double ***) mxMalloc(num_c * sizeof(double **));

   for (c = 0; c < num_c; c++)
   {
      r[c] = (double **) mxMalloc(num_s * sizeof(double *));

      for (s = 0; s < num_s; s++)
      {
	 r[c][s] = (double *) mxMalloc(max_t * sizeof(double));

	 if (!r[c][s])
	 {
	    fprintf(stderr, "Unable to malloc at c = %d, s = %d.\n", c, s);
	    exit(-1);
	 }
      }
		 ...
		 *prhs[])
{
  ...
  responses = (double **)calloc(num_trials, sizeof(double *));
  for(i = 0; i < num_trials; i++)
    responses[i] = calloc(num_c, sizeof(double));
