function val = RasterImagePlot2(S, varargin)

% RasterImagePlot2(S, height, bar, tstart, tend)
% 
% Plot a continuous function in a spike raster like form. 
% RasterImagePlot2 plots each waveformin a 3D landscape style
% INPUT:
...
for i=1:size(matVal,1)
	plot(times/10000,matVal(i,:)+3*i,'Color',[i/length(Si) 0.5 (1-i/length(Si))])
end
